...
freqs_output_file = open(sys.argv[4], 'w')
freqs_output_file.write("locus\tPohang\tGeoje\tNamhae\tYellowSea\tJukbyeon\tJinBay07\tJinBay08\tJinBay08\tBoryeong\tGeoje14\n")
...
for mystring in genotypes_file:		# Read in each line in the file as a string
	if count == 0:
		...
		JinhaeBay08 = stripped_string.split(' ')[196:240]
		Boryeong07 = stripped_string.split(' ')[240:264]
		Geoje14 = stripped_string.split(' ')[264:299]
...
#write loci to appropriate file
		missing_data_freqs_out = [str(i) for i in overall_percent_missingdata]
		freqs_output_file.write(locus_name + "\t" + "\t".join(missing_data_freqs_out) + "\n")
		if all(i < 0.30 for i in overall_percent_missingdata):
